				...
				for(; *arg_p; ++arg_p)
					if ('-' == (*arg_p)[0] && ((*arg_p)[1] != (char) 0)) {
						if ('-' == (*arg_p)[1]) {
							...
							for (; opt_spec_p->key != 0; ++opt_spec_p)
								if (strchr(opt_spec_p->shorts, (*arg_p)[1])) {
									opt_count += (
										opt_spec_p->flags & GOPT_ARG ?
										1 : strlen((*arg_p) + 1));
									break;
								}
						}
...
int64_t gopt(const void *vptr_opts, int key)
{
...
int64_t gopt_arg(const void *vptr_opts, int key, const char **arg)
{
	const opt_t *opts = vptr_opts;
	int64_t count = 0;
...
const char *gopt_arg_i(const void *vptr_opts, int key, int64_t i)
{
	const opt_t *opts = vptr_opts;
...
int64_t gopt_args(const void *vptr_opts, int key, const char **args, int64_t args_len)
{
